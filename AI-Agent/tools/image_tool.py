...
class ImageTool:
    ...
    def run(
        ...
        try:
            ...
            if status_code == 429:
                LOGGER.warning("Gemini image quota is unavailable for action '%s'", action)
                result = failure(
                    "The configured Gemini project has no image-generation quota available.",
                    "Gemini image quota exceeded",
                )
