...
for key_index in range(1, 6):
    api_key = os.getenv(f"GEMINI_API_KEY_{key_index}")
    if api_key:
...
def get_task_history() -> Any:
    with state_lock:
        active = [
            {
                "run_id": run_id,
                "status": "waiting_for_approval" if item.get("waiting") else "running",
...
def allow_action() -> Any:
    ...
    approval_store.decide(request_id, approved, meta={"remote_addr": request.remote_addr})
...
def agent_chat() -> Any:
    ...
    try:
        data = request.get_json(silent=True) or {}
        requested_run_id = str(data.get("run_id", "")).strip()
        with state_lock:
        ...
        for attempt in range(total_clients):
            ...
            try:
                ...
                task["next_step"] = result.get("next_step", result.get("final_step", task["next_step"]))
                task["updated_at"] = time.time()
                    ...
                    return jsonify(
                        {
                            ...
                            "tool_events": task["tool_events"],
                            "progress": min(95, int((task["next_step"] / 24) * 100)),
                        }
                    )
                ...
                with state_lock:
                    ...
                    task_sessions.pop(run_id, None)
                status_code = 200 if completed else 500
                return (
                    jsonify(
                        {
                            "success": completed,
                ...
                last_error = exc
                LOGGER.warning("Gemini slot %s failed: %s", key_number, exc, exc_info=True)
                continue

        return jsonify({"success": False, "run_id": run_id, "error": f"All Gemini client slots failed: {last_error}"}), 502
    except ValueError as exc:
        return jsonify({"success": False, "error": str(exc)}), 400
    except Exception as exc:
        LOGGER.exception("Unhandled /agent/chat failure")
        return jsonify({"success": False, "error": f"Agent gateway error: {exc}"}), 500
