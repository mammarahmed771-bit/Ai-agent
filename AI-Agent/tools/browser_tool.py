...
class BrowserTool:
    ...
    async def _dispatch(
        ...
        if action == "open_url":
            ...
            try:
                ...
                if self._is_stale_page_error(exc):
                    # Retry once after recovering page.
                    page = await self._recover_page_if_needed(exc)
                    await page.goto(url, wait_until=wait_until, timeout=timeout_ms)
                else:
                    raise
    ...
    async def _page_result(
        ...
        # NOTE: This must be resilient to stale/closed pages after navigations (e.g. Google).
        # We reuse the existing browser/context; only create a new page if needed.
        page = self._get_page()
        data = {"url": page.url, "title": await page.title(), "active_index": self._active_index}
        data.update(extra or {})
        return success(message, data)

    def _is_stale_page_error(self, exc: BaseException) -> bool:
        # Playwright error messages can vary. We keep this heuristic lightweight and targeted
        # to the reported "page/session becomes stale after navigation" issue.
        msg = f"{exc}".lower()
        patterns = (
            "target closed",
            "page closed",
    ...
    def _get_page(self) -> Any:
        """
        Returns an active Playwright page that is safe to use.
        Reuses the existing browser/context when possible and creates a new page only when
        the current one is invalid/closed.
        """
        if self._context is None:
        ...
        if not pages:
            # No pages left: create a fresh one.
            self._active_index = 0
            return self._context.new_page()  # may raise; caller is in async flow

        # Clamp active index.
        self._active_index = max(0, min(self._active_index, len(pages) - 1))

        # If the active page is closed, pick the first open page.
        page = pages[self._active_index]
        try:
            if hasattr(page, "is_closed") and page.is_closed():
                raise RuntimeError("Active page is closed")
        except Exception:
            # Fallback: find any open page.
            for i, candidate in enumerate(pages):
                try:
                    if hasattr(candidate, "is_closed") and candidate.is_closed():
                        continue
                except Exception:
                    # If is_closed isn't reliable, just skip this candidate.
                    continue
                self._active_index = i
                return candidate

            # No open pages: create a new one in the same context.
            self._active_index = 0
            return self._context.new_page()

        return page

    def _choose_existing_page_or_new(self) -> Any:
        """
        Synchronous helper used from recovery code paths where we already know we’re in a valid async context.
        Picks an open page (if any); otherwise returns None so the caller can create a new page.
        """
        if self._context is None:
            raise RuntimeError("Browser is not running")
        ...
        # Prefer active page if open; otherwise first open page.
        preferred = pages[self._active_index]
        try:
            if hasattr(preferred, "is_closed") and preferred.is_closed():
                raise RuntimeError("Active page is closed")
            return preferred
        except Exception:
            for i, candidate in enumerate(pages):
    ...
    async def _recover_page_if_needed(self, exc: BaseException) -> Any:
        """
        Attempt recovery only for known stale/closed page errors.
        Recreates only the page (or browser/context if necessary).
        """
        if not self._is_stale_page_error(exc):
            raise exc

        LOGGER.warning(
            "Recovering from stale/closed page error; active_index=%s error=%s",
            self._active_index,
            exc,
        )

        # If context is broken/disconnected, recreate whole browser/context.
        if self._context is None:
            await self._launch({}, {})
            return self._get_page()

        # If there are still pages, try to pick an existing one.
        try:
            existing = self._choose_existing_page_or_new()
            if existing is not None:
                return existing
        except Exception:
            LOGGER.exception("Failed selecting existing page during recovery")

        # Otherwise create a new page within the same context.
        try:
            await self._context.new_page()
            self._active_index = len(self._context.pages) - 1
            return self._context.pages[self._active_index]
        except Exception:
            LOGGER.exception("Failed creating a new page in existing context; recreating browser/context")
            await self._close_browser()
            await self._launch({}, {})
            return self._get_page()

    def _page(self) -> Any:
        # Backwards-compat internal selector: delegate to the resilient implementation.
        return self._get_page()
