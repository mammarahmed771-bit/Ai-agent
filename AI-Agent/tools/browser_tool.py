...
class BrowserTool:
    """Persistent, multi-tab Playwright browser controlled from a worker loop."""

    name = "browser"
    description = (
        "Launch and control a persistent Chromium browser. Supports tabs, open_url, "
    ...
    def _permission_for(
        ...
        if action == "upload":
            paths = [str(resolve_safe_path(str(path), session_state, must_exist=True)) for path in self._as_list(args.get("paths", args.get("path", [])))]
    ...
    async def _dispatch(
        ...
        if action == "open_url":
            ...
            await page.goto(url, wait_until=str(args.get("wait_until", "domcontentloaded")), timeout=timeout_ms)
    ...
    async def _launch(self, args: Dict[str, Any], session_state: Dict[str, Any]) -> None:
        ...
        try:
            self._context = await self._playwright.chromium.launch_persistent_context(
                ...
                headless=bool(args.get("headless", os.getenv("BROWSER_HEADLESS", "false").lower() == "true")),
                accept_downloads=True,
                viewport={"width": int(args.get("width", 1440)), "height": int(args.get("height", 900))},
            )
        except Exception:
