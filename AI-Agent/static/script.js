...
function setActionRequestModalVisible(visible) {
    const els = getActionRequestModalEls();
    if (!els.modal) return;
    els.modal.style.display = visible ? "flex" : "none";
    els.modal.setAttribute("aria-hidden", visible ? "false" : "true");
}

function setActionRequestButtonsDisabled(disabled) {
    const els = getActionRequestModalEls();
    if (els.allowBtn) els.allowBtn.disabled = disabled;
    if (els.denyBtn) els.denyBtn.disabled = disabled;
    if (els.closeBtn) els.closeBtn.disabled = disabled;
}

async function submitActionDecision(approved) {
    const actionRequest = window.__current_action_request__;
    const requestId = String(actionRequest?.request_id || "").trim();
    const runId = String(actionRequest?.run_id || actionRequest?.payload?.run_id || "").trim();
    if (!requestId || !runId) {
        throw new Error("The pending action is missing its request or run identifier.");
    }

    setActionRequestButtonsDisabled(true);
    try {
        const response = await fetch("/agent/allow_action", {
            method: "POST",
        ...
        setActionRequestModalVisible(false);
        window.__current_action_request__ = null;
        await resumeAgentAfterPermission(runId);
    } finally {
        setActionRequestButtonsDisabled(false);
    }
}

async function handleActionRequest(actionRequest, runId = "") {
    const els = getActionRequestModalEls();
    if (!els.modal) {
    ...
    window.__current_action_request__ = {
        ...actionRequest,
        run_id: runId || actionRequest?.run_id || actionRequest?.payload?.run_id || ""
    };

    const reqId = window.__current_action_request__.request_id || "";
    ...
    if (els.denyBtn && !els.denyBtn.__bb_bound) {
        ...
        els.denyBtn.addEventListener("click", async () => {
            try {
                await submitActionDecision(false);
            } catch (e) {
                generateAIResponse(`❌ Permission denial failed: ${e.message || e}`, performance.now());
            }
        });
    }
...
async function resumeAgentAfterPermission(runId) {
    if (!runId) throw new Error("Cannot resume the agent without its original run_id.");
    ...
    if (!data) throw new Error(`Server returned a non-JSON response (${res.status}).`);

    if (data.success) {
        generateAIResponse(data.reply || "", performance.now());
    } else if (data.action_request) {
        await handleActionRequest(data.action_request, data.run_id || runId);
    } else {
        generateAIResponse(`⚠️ Agent error: ${data.error || `Server error ${res.status}`}`, performance.now());
    }
}
...
function sendMessage(){
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            handleActionRequest(data.action_request, data.run_id);
        } else {
